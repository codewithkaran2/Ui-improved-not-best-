...
// Arrays for additional features
let bullets = [];
let powerUps = [];
let traps = [];
let explosions = [];
...
const player1 = {
  ...
  isDashing: false // For dash visual effect
};
const player2 = {
  x: 600,
  y: 0,
...
const dashDuration = 200;  // dash visual effect duration
...
function dash(player) {
  ...
  // Keep player within canvas bounds and above UI (y >= 140)
  ...
  player.isDashing = true; // Activate dash glow effect
...
document.addEventListener("keydown", (e) => {
  ...
  // Dash controls: Player 1 uses "E", Player 2 uses "O"
...
function rectCollisionGeneric(a, b) {
  const r1 = { x: a.x, y: a.y, width: a.width || a.size, height: a.height || a.size };
  const r2 = { x: b.x, y: b.y, width: b.width || b.size, height: b.height || b.size };
...
function spawnPowerUp() {
  ...
  const y = Math.random() * (canvas.height - size - 150) + 150;
  const duration = 5000; // 5-second lifetime
...
function drawPowerUps() {
  powerUps.forEach((p) => {
    ...
    ctx.fillStyle = "white";
    ctx.font = "18px Montserrat, sans-serif"; // Use Montserrat for consistency
...
function drawTopStatus() {
  ...
  ctx.fillText("Score: " + p1Score + " - " + p2Score, canvas.width / 2, 40);
  ctx.fillStyle = "blue";
...
function drawPlayers() {
  ...
  if (player1.isDashing) {
    ctx.save();
    ctx.globalAlpha = 0.5;
    ctx.fillStyle = "cyan";  // Glow for player1 dash
  ...
  if (player2.isDashing) {
    ctx.save();
    ctx.globalAlpha = 0.5;
    ctx.fillStyle = "yellow";  // Glow for player2 dash
...
function gameLoop() {
  ...
  // Draw traps, power-ups, players, and UI
  drawTraps();
  drawPowerUps();
  drawPlayers();
  drawTopStatus();
